...
class Qix extends DisplayableBase {
  Vst vst;
  ...
  float vx0 = random(5, 10);
  float vy0 = random(5, 10);
  float vx1 = random(4, 10);
  float vy1 = random(4, 10);
  int head = 0;
  ...
  void display() {
    for (int i = 0; i < segments; i++) {
      vst.vline(i == head, x0[i], y0[i], x1[i], y1[i]);
    }
  }
}